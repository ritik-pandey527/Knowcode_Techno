...
esp32_port = '/dev/ttyUSB0'  # Replace with your ESP32's port (e.g., 'COM3', '/dev/ttyUSB0')

baud_rate = 115200   # Same baud rate as in the ESP32 sketch
...
# Variables for eye-closed duration tracking

gesture_name = ""  # Initialize gesture_name

system_on = False

eye_closed_duration = 0  # Duration the eyes have been closed

eye_state_start_time = None  # Start time when eyes are closed
...
try:
    ...
    time.sleep(0.2)  # Allow some time for the ESP32 to initialize
    ...
    while cap.isOpened():
        ...
        if result.multi_face_landmarks:
            ...
            for face_landmarks in result.multi_face_landmarks:
                ...
                # Draw landmarks and process eye gestures
                ...
                if gesture_name == "both close":

                    if eye_state_start_time is None:

                        eye_state_start_time = time.time()  # Start timing
                    ...
                    if eye_closed_duration > eye_closed_threshold:
                        ...
                        eye_state_start_time = None  # Reset timing

                        print(f"System {'ON' if system_on else 'OFF'}")

                else:

                    eye_state_start_time = None  # Reset if not "both close"

                    eye_closed_duration = 0  # Reset duration
                ...
                # Display the eye-closed duration
